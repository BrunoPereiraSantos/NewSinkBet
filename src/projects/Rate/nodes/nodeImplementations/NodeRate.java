...
public class NodeRate extends Node implements InterfaceRequiredMethods {
	...
	private void handleHello(Node sender, Node receiver,
			GenericWeightedEdge incomingEdge, RateHelloMessage msg) {
		...
		if ((msg.getMtmPath() + edgeToSender.getMtm() < mtmPath)
			...
			// verifica se tem alguma msg na fila, caso tenha atualiza o timer
			// em mais 1s,
	...
	public void handleEvent(Inbox inbox, EventMessage msg) {
		...
		if ((msg.getNextHop() == 1) && (this.ID == 1)) {
			...
			statistics.IncomingEvents(msg.idSender, Tools.getGlobalTime()
					- msg.firedTime);
			return;
		}
