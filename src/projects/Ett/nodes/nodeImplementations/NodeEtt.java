...
public class NodeEtt extends Node {
		...
		while (inbox.hasNext()) {
			...
			if (m instanceof EttHelloMessage) {
				...
				handleHello(inbox.getSender(), inbox.getReceiver(),
						(EdgeEtt) inbox.getIncomingEdge(), msg);
			}
	...
	private void handleHello(Node sender, Node receiver, EdgeEtt incomingEdge,
			EttHelloMessage msg) {
	...
	public void init() {
		...
		if (this.ID == 1) {
			...
			this.pathEtt = 0;
			EttHelloMessage msg = new EttHelloMessage(0, this.ID, this.pathEtt);
	...
	public void start() {
		...
		EttHelloMessage hellomsg = new EttHelloMessage(0, this.ID, 0);
