...
import projects.BetEtx.nodes.messages.PackHelloBetEtx;
import projects.BetEtx.nodes.messages.PackReplyEtx;
import projects.BetEtx.nodes.timers.TimerFwdReplyBetEtx;
import projects.BetEtx.nodes.timers.TimerSendHelloBetEtx;
import projects.BetEtx.nodes.timers.TimerStartReplyFlood;
import projects.BetEtx.nodes.timers.TimerStartSimulationBetEtx;
...
public class NodeBetEtx extends Node {
	...
	TimerSendHelloBetEtx fhp = new TimerSendHelloBetEtx();
	...
	public void handleMessages(Inbox inbox) {
		while (inbox.hasNext()) {
			...
			if (msg instanceof PackHelloBetEtx) {
				
				PackHelloBetEtx a = (PackHelloBetEtx) msg;
	 ...
	 * ============================================================
	 */
	public void handlePackHello(Node sender, Node receiver, PackHelloBetEtx message) {
	...
	public void fwdHelloPack() {
		...
		broadcast(new PackHelloBetEtx(hops, pathsToSink, this.ID, sinkID, etxPath)); 
		setSentMyHello(true);
	}
	...
	public void sendHelloFlooding() {
		...
		broadcast(new PackHelloBetEtx(hops, 1, this.ID, this.ID, 0.0));
		setSentMyHello(true);
		
	}

	public void sendReplyFlooding(){ //Dispara o flooding das respostas dos nodos com papel BORDER e RELAY
		if ((getRole() == NodeRoleBetEtx.BORDER) || 
			(getRole() == NodeRoleBetEtx.RELAY)) {
	...
	public void handlePackReply(Node sender, Node receiver, PackReplyEtx message) {
		...
		if (!sons.contains(message.getSenderID()) && 
			...
			TimerFwdReplyBetEtx fwdReply = new TimerFwdReplyBetEtx(message);
	...
	public void init() {
		...
		if (this.ID == 1) {
			...
			setRole(NodeRoleBetEtx.SINK);
			
			(new TimerStartSimulationBetEtx()).startRelative(3, this);
	 ...
	 *							Gets e Sets
	 *********************************************************************************/
	public NodeRoleBetEtx getRole() {return role;}
	public void setRole(NodeRoleBetEtx role) {this.role = role;}
	...
	public double getEtxToMeFromNode(int nodeID) {
		...
		while (it2.hasNext()) {
			e = (EdgeBetEtx) it2.next();
			if (e.endNode.ID == nodeID){
				e = (EdgeBetEtx) e.getOppositeEdge();
				return e.getEtx();
			}
		}
		return 0.0;
