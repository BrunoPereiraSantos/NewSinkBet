package projects.OldBetHop.nodes.nodeImplementations;
...
import projects.OldBetHop.nodes.edges.EdgeOldBetHop;
import projects.OldBetHop.nodes.messages.PackHelloOldBetHop;
import projects.OldBetHop.nodes.messages.PackReplyOldBetHop;
import projects.OldBetHop.nodes.timers.TimerFwdReplyOldBetHop;
import projects.OldBetHop.nodes.timers.TimerSendHelloOldBetHopSbet;
import projects.OldBetHop.nodes.timers.TimerStartReplyFloodOldBetHop;
import projects.OldBetHop.nodes.timers.TimerStartSimulationOldBetHop;
...
public class NodeOldBetHop extends Node {

	
	// Qual o papel do nodo.
	private NodeRoleOldBetHopSbet role;
	...
	TimerSendHelloOldBetHopSbet fhp = new TimerSendHelloOldBetHopSbet();
	TimerStartReplyFloodOldBetHop srf = new TimerStartReplyFloodOldBetHop();
	...
	public void handleMessages(Inbox inbox) {
		...
		while (inbox.hasNext()) {
			...
			if (msg instanceof PackHelloOldBetHop) {
				...
				handlePackHello(a);
			}else if (msg instanceof PackReplyOldBetHop) {
				PackReplyOldBetHop b = (PackReplyOldBetHop) msg;
				handlePackReply(b);
			}
		}
	}
	 ...
	 * ============================================================
	 */
	public void handlePackHello(PackHelloOldBetHop message) {
	...
	public void fwdHelloPack() {
		...
		broadcast(new PackHelloOldBetHop(hops, pathsToSink, this.ID, sinkID)); 
		setSentMyHello(true);
	...
	public void sendHelloFlooding() {
		...
		broadcast(new PackHelloOldBetHop(hops, 1, this.ID, this.ID));
		setSentMyHello(true);
	 ...
	 * ============================================================
	 */
	public void handlePackReply(PackReplyOldBetHop message) {
		...
		if (!sons.contains(message.getSenderID()) && 
			...
			setRole(NodeRoleOldBetHopSbet.RELAY);
			...
			TimerFwdReplyOldBetHop fwdReply = new TimerFwdReplyOldBetHop(message);
	...
	public void fwdReply(PackReplyOldBetHop pkt){//Dispara um broadcast com o pacote PackReplyHopSbet | metodo utilizado pelos timers
		broadcast(pkt);
	}
	...
	@Override
	public void init() {		
		setRole(NodeRoleOldBetHopSbet.BORDER);
		setPathsToSink(0);
		setHops(0);
		setsBet(0.0);
		...
		if (this.ID == 1) {
			...
			setRole(NodeRoleOldBetHopSbet.SINK);
			
			(new TimerStartSimulationOldBetHop()).startRelative(1, this);
	 ...
	 *							Gets e Sets
	 *********************************************************************************/
	public NodeRoleOldBetHopSbet getRole() {return role;}
	public void setRole(NodeRoleOldBetHopSbet role) {this.role = role;}
	...
	public double getEtxToMeFromNode(int nodeID) {
		...
		EdgeOldBetHop e;
		while (it2.hasNext()) {
			e = (EdgeOldBetHop) it2.next();
			if (e.endNode.ID == nodeID){
				e = (EdgeOldBetHop) e.getOppositeEdge();
				return e.getEtx();
			}
		}
		return 0.0;
	...
	public TimerSendHelloOldBetHopSbet getFhp() {
		return fhp;
	}

	public void setFhp(TimerSendHelloOldBetHopSbet fhp) {
		this.fhp = fhp;
	}

	public TimerStartReplyFloodOldBetHop getSrf() {
		return srf;
	}

	public void setSrf(TimerStartReplyFloodOldBetHop srf) {
		this.srf = srf;
	}
