package projects.OldBetEtx.nodes.nodeImplementations;
...
import projects.OldBetEtx.nodes.messages.PackHelloOldBetEtx;
import projects.OldBetEtx.nodes.messages.PackReplyOldBetEtx;
import projects.OldBetEtx.nodes.timers.TimerFwdReplyOldBetEtx;
import projects.OldBetEtx.nodes.timers.TimerSendHelloOldBetEtx;
import projects.OldBetEtx.nodes.timers.TimerStartReplyFloodOldBetEtx;
import projects.OldBetEtx.nodes.timers.TimerStartSimulationOldBetEtx;
...
public class NodeOldBetEtx extends Node {
	// Qual o papel do nodo
	private NodeRoleOldBetEtx role;
	...
	TimerSendHelloOldBetEtx fhp = new TimerSendHelloOldBetEtx();
	TimerStartReplyFloodOldBetEtx srf = new TimerStartReplyFloodOldBetEtx();
	
	@Override
	public void handleMessages(Inbox inbox) {
		while (inbox.hasNext()) {
			Message msg = inbox.next();
			if (msg instanceof PackHelloOldBetEtx) {
				
				PackHelloOldBetEtx a = (PackHelloOldBetEtx) msg;
				handlePackHello(inbox.getSender(), inbox.getReceiver(), a);
				
			}else if (msg instanceof PackReplyOldBetEtx) {
				PackReplyOldBetEtx b = (PackReplyOldBetEtx) msg;
	 ...
	 * ============================================================
	 */
	public void handlePackHello(Node sender, Node receiver, PackHelloOldBetEtx message) {
	...
	public void fwdHelloPack() {
		...
		broadcast(new PackHelloOldBetEtx(hops, pathsToSink, this.ID, sinkID, etxPath)); 
		setSentMyHello(true);
	}
	...
	public void sendHelloFlooding() {
		...
		broadcast(new PackHelloOldBetEtx(hops, 1, this.ID, this.ID, 0.0));
		setSentMyHello(true);
	...
	public void sendReplyFlooding(){ //Dispara o flooding das respostas dos nodos com papel BORDER e RELAY
		if ((getRole() == NodeRoleOldBetEtx.BORDER) || 
			...
			PackReplyOldBetEtx pkt = new PackReplyOldBetEtx(hops, pathsToSink, this.ID, sinkID, nextHop, neighbors, etxPath, sBet, this.ID);
			broadcast(pkt);
	...
	public void handlePackReply(Node sender, Node receiver, PackReplyOldBetEtx message) {
		...
		if (!sons.contains(message.getSenderID()) && 
			...
			setRole(NodeRoleOldBetEtx.RELAY);
			...
			TimerFwdReplyOldBetEtx fwdReply = new TimerFwdReplyOldBetEtx(message);
	...
	public void processBet(PackReplyOldBetEtx message) {	// faz o cal. do Sbet
	...
	public void fwdReply(PackReplyOldBetEtx pkt){//Dispara um broadcast com o pacote PackReplyEtxBet | metodo utilizado pelos timers
		broadcast(pkt);
	}
	...
	@Override
	public void init() {
		setRole(NodeRoleOldBetEtx.BORDER);
		setPathsToSink(0);
		setHops(0);
		setsBet(0.0);
		...
		if (this.ID == 1) {
			...
			setRole(NodeRoleOldBetEtx.SINK);
			
			(new TimerStartSimulationOldBetEtx()).startRelative(3, this);
	 ...
	 *							Gets e Sets
	 *********************************************************************************/
	public NodeRoleOldBetEtx getRole() {return role;}
	public void setRole(NodeRoleOldBetEtx role) {this.role = role;}
